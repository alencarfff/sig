...
require('poo/app/Library/PHPMailer/class.phpmailer.php');

/**
 * Email [ MODEL ]
 * Modelo responÃ¡vel por configurar a PHPMailer, validar os dados e disparar e-mails do sistema!
 * 
 */
class Email {
    ...
    public function Enviar(array $Data) {
        ...
        elseif (!Check::Email($this->Data['RemetenteEmail'])):
            $this->Error = ['Erro ao enviar mensagem: O e-mail que vocÃª informou nÃ£o tem um formato vÃ¡lido. Informe seu E-mail!', WS_ALERT];
            $this->Result = false;
        else:
            $this->setMail();
            $this->Config();
